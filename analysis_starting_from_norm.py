...
from plots import plot_contours, plot_areas, pca_animation1, plot_trajectory_animation, plot_trajectory_animation_diff
...
def get_shape_space_from_trajectory_space(trajectory_space_vectors, trajectory_space_map, shape_space_shape):
    new_shape = (
        len(trajectory_space_vectors), *shape_space_shape) if trajectory_space_vectors.ndim > 1 else shape_space_shape
    shape_space_vectors = trajectory_space_map.inverse_transform(trajectory_space_vectors).reshape(*new_shape)
    return shape_space_vectors


def get_original_space_from_shape_space(shape_space_vectors, shape_space_map, original_space_shape):
    new_shape = (
        len(shape_space_vectors), *original_space_shape) if shape_space_vectors.ndim > 2 else original_space_shape
    original_space_vectors = shape_space_map.inverse_transform(shape_space_vectors).reshape(*new_shape)
    return original_space_vectors


def get_original_space_from_trajectory_space(trajectory_space_vectors, trajectory_space_map, shape_space_shape,
                                             shape_space_map, original_space_shape):
    shape_space_vectors = get_shape_space_from_trajectory_space(trajectory_space_vectors, trajectory_space_map,
                                                                shape_space_shape)
    original_space_vectors = get_original_space_from_shape_space(shape_space_vectors, shape_space_map,
...
if __name__ == '__main__':
    ...
    contours_time_interpolated = prepare_contours(
        contours, scale=False, center=False,
        interpolate_time_points=n_time_points,
        fourier_interpolate_kwargs=None,
        close_contours=False
    )
    contours_fourier_interpolated = prepare_contours(
        contours_time_interpolated, scale=False, center=False, interpolate_time_points=None,
        fourier_interpolate_kwargs={'n_harmonics': n_harmonics,
                                    'n_points_out': None
                                    },
        close_contours=False
    )
    ...
    np.save(data_dir.joinpath('contours_fourier_interpolated'), contours_fourier_interpolated)
    ...
    contour_names, shape_series = get_contour_tensor(contours_fourier_interpolated, only_endo=True)
    ...
    norm_check = [1 if 'norm' in name else 0
    ...
    print(contour_names)
    global_mean, norm_gpa_shapes = shape_analysis.generalized_procrustes_analysis(norm_shapes, scale=scale)
    ...
    pathology_shapes = pathology_shape_series.reshape(-1, *pathology_shape_series.shape[2:])
    ...
    norm_gpa_shape_series = np.array(norm_gpa_shapes).reshape(*norm_shape_series.shape)
    pathology_gpa_shape_series = pathology_gpa_shapes.reshape(*pathology_shape_series.shape)
    ...
    gpa_shape_series[np.where(np.logical_not(norm_check))] = pathology_gpa_shape_series[:]
    gpa_shapes = gpa_shape_series.reshape(*shapes.shape)
    print('gpa_shapes', len(np.unique(gpa_shapes, axis=0)))

    ls_shape_series = shape_analysis.linear_shift(shape_series, global_mean, scale=scale)
    ...
    new_global_mean, norm_lsgpa_shapes = shape_analysis.generalized_procrustes_analysis(norm_ls_shapes, scale=scale)
    pathology_ls_shape_series = ls_shape_series[np.where(np.logical_not(norm_check))]
    pathology_ls_shapes = pathology_ls_shape_series.reshape(-1, *pathology_ls_shape_series.shape[2:])
    ...
    pathology_lsgpa_shape_series = np.array(pathology_lsgpa_shapes).reshape(*pathology_ls_shape_series.shape)
    lsgpa_shape_series = np.zeros_like(ls_shape_series)
    lsgpa_shape_series[np.where(norm_check)] = norm_lsgpa_shape_series
    lsgpa_shape_series[np.where(np.logical_not(norm_check))] = pathology_lsgpa_shape_series
    ...
    print('lsgpa_shape_series', len(np.unique(lsgpa_shape_series, axis=0)), lsgpa_shape_series.shape)

    shapes_list = [shapes, gpa_shapes, ls_shapes, lsgpa_shapes]
    shape_series_list = [shape_series, gpa_shape_series, ls_shape_series, lsgpa_shape_series]
    names = ['raw', f'gpa{"" if scale else "_noscale"}', f'linear_shifted{"" if scale else "_noscale"}',
    ...
    bounds = np.array([np.min(aligned_shape_series, axis=(0, 1, 2)), np.max(aligned_shape_series, axis=(0, 1, 2))])
    ...
    norm_aligned_shapes = norm_aligned_shape_series.reshape(-1, *norm_aligned_shape_series.shape[2:])
    print('norm_aligned_shapes', len(np.unique(norm_aligned_shapes, axis=0)), norm_aligned_shapes.shape)
    pca1.fit(norm_aligned_shapes.reshape(norm_aligned_shapes.shape[0], -1))
    trajectories = pca1.transform(aligned_shape_series.reshape(np.product(aligned_shape_series.shape[:2]), -1)).reshape(
        *lsgpa_shape_series.shape[:2], -1)
    ...
    plt.close()
    last_scale = False
    mean_trajectory, norm_trajectories_gpa = shape_analysis.generalized_procrustes_analysis(
        trajectories[np.where(norm_check)],
        scale=last_scale)
    pathology_trajectories_gpa = [
        shape_analysis.procrustes_analysis(trajectory, mean_trajectory, scale=last_scale)[1]
    ...
    pca2.fit(trajectories_gpa.reshape(trajectories_gpa.shape[0], -1)[np.where(norm_check)])
    final_space = pca2.transform(trajectories_gpa.reshape(trajectories_gpa.shape[0], -1))
    ...
    np.save(result_dir.joinpath('space_shape_variance.npy'), pca1.explained_variance_ratio_)
    np.save(result_dir.joinpath('space_traj_components.npy'), pca2.components_)
    np.save(result_dir.joinpath('space_traj_variance.npy'), pca2.explained_variance_ratio_)
    # raise ValueError
    ...
    bad_pairs = []
    pairs = {key: pairs[key] for key in pairs
             if ('after' in pairs[key]) and ('before' in pairs[key])}

    before_operation_index = [i for i, name in enumerate(contour_names) if name.endswith('np')]
    after_operation_index = [i for i, name in enumerate(contour_names) if name.endswith('0m')]
    ...
    for operation_status in ('norm', 'np', '0m'):
        plt.scatter(*final_space[:, :2][np.where(np.char.endswith(contour_names, operation_status))].T,
    ...
    for pair in pairs.values():
        ...
        dv = arrow - tail
        plt.arrow(*tail, *dv, length_includes_head=True, linestyle='--', color='black', alpha=0.3, head_width=0.01,
                  head_length=0.03)
    ...
    plt.figure(figsize=(10, 10))
    for operation_status in ('norm', 'np', '0m'):
        plt.scatter(*final_space[:, 1:3][np.where(np.char.endswith(contour_names, operation_status))].T,
                    label=operation_status)
    # for cluster in np.unique(clusters):
    #     ax = plt.scatter(*final_space[:, :2][np.where(clusters == cluster)].T, label={cluster})
    for i, name in enumerate(contour_names):
        plt.gca().annotate(name.split('_')[0], final_space[i, 1:3])
    for pair in pairs.values():
        tail = final_space[:, 1:3][pair['before']]
        arrow = final_space[:, 1:3][pair['after']]
        dv = arrow - tail
        plt.arrow(*tail, *dv, length_includes_head=True, linestyle='--', color='black', alpha=0.3, head_width=0.01,
                  head_length=0.03)
    plt.xlabel(f'PCA 2 {pca2.explained_variance_ratio_[1]:.2f}%')
    plt.ylabel(f'PCA 3 {pca2.explained_variance_ratio_[2]:.2f}%')
    plt.legend()
    plt.gca().set_aspect('equal')
    plt.savefig(result_dir.joinpath(f'{names[3]}/trajectory_space_scatterplot2_3.png'))
    plt.close()
    ...
    mean_trajectory_in_shape_space = get_shape_space_from_trajectory_space(mean_trajectory_in_trajectory_space, pca2,
                                                                           trajectories_gpa.shape[1:])
    print('shape space shape', mean_trajectory_in_shape_space.shape)
    mean_trajectory_in_original_space = get_original_space_from_shape_space(mean_trajectory_in_shape_space, pca1,
    ...
    final_space_bounds = np.array([np.min(final_space, axis=0), np.max(final_space, axis=0)])
    n_points_per_component = 2
    for component in range(final_space.shape[1]):
        component_movement = np.concatenate(
            [np.linspace(final_space_bounds[0, component], 0, n_points_per_component, endpoint=False),
             np.linspace(0, final_space_bounds[1, component], n_points_per_component + 1)])
        trajectories_ = np.array([mean_trajectory for i in range(len(component_movement))])
        for i in range(len(component_movement)):
            trajectories_[i, component] = component_movement[i]
        print(trajectories_.shape)
        trajectories_shapes = get_shape_space_from_trajectory_space(trajectories_, pca2, trajectories_gpa.shape[1:])
        scalar_mappable = ScalarMappable(mcolors.Normalize(vmin=component_movement[0], vmax=component_movement[-1]),
                                         'coolwarm')
        print(trajectories_shapes.shape)
        for i, shape in enumerate(trajectories_shapes):
            x, y = shape.T[:2]
            plt.plot(x, y, marker='o', color=scalar_mappable.to_rgba(component_movement[i]))
            plt.scatter(x[0], y[0], color='black', zorder=2, marker='x')
        plt.scatter(*trajectories_shapes[n_points_per_component + 1, 0, :2].T, color='black',
                    label='First point in cycle', zorder=3, marker='x')
        plt.xlim(-1, 1)
        plt.ylim(-1, 1)
        plt.xlabel('Shape Space component 1')
        plt.ylabel('Shape Space component 2')
        plt.title(f'Changes in {component + 1} component of Trajectories Space in Shape Space')
        plt.legend()
        plt.colorbar(scalar_mappable, orientation='horizontal', label='Some Units'
                     )
        plt.gca().set_aspect('equal')
        plt.savefig(result_dir.joinpath(f'{names[3]}/trajectory_space_component_{component + 1}_in_shape_space.png'))
        plt.close()
    ...
    final_space_bounds = np.array([np.min(final_space, axis=0), np.max(final_space, axis=0)]) * 2
    print(final_space_bounds.shape)
    mean_trajectory = np.zeros_like(final_space[0])
    mean_trajectory_shape = get_original_space_from_trajectory_space(mean_trajectory, pca2, trajectories_gpa.shape[1:],
    ...
    #     min_trajectory = get_original_space_from_trajectory_space(min_trajectory, pca2, trajectories_gpa.shape[1:],
    #                                                               pca1, aligned_shape_series.shape[1:])
    ...
    #     max_trajectory = get_original_space_from_trajectory_space(max_trajectory, pca2, trajectories_gpa.shape[1:],
    #                                                               pca1, aligned_shape_series.shape[1:])
    ...
    for i in (3,):
        ...
        pca_animation1(shapes_list[i].shape, pca=pca1, pca_space=trajectories.reshape(-1, *trajectories.shape[2:]),
                       out_dir=pca_1_dir, n_frames=10,
                       interval=50)
    ...
    for i in (3,):
        ...
        print(shapes.shape)
        mean_trajectory_shapes = mean_trajectory_shapes.reshape(-1, *shapes[0].shape).transpose(0, 2, 1)
        ...
        plt.plot(*np.mean(norm_trajectories_gpa, axis=0)[:, :2].T, color='black', marker='o',
                 label='mean trajectory (norm')
        plt.plot(*np.mean(pathology_trajectories_gpa, axis=0)[:, :2].T, color='red', marker='o',
