...
if __name__ == '__main__':
    ...
    scale = True
    global_mean, gpa_shapes = shape_analysis.generalized_procrustes_analysis(shapes, scale=scale)
    ...
    new_global_mean, lsgpa_shapes = shape_analysis.generalized_procrustes_analysis(ls_shapes, scale=scale)
    ...
    bounds = np.array([np.min(aligned_shapes, axis=(0, 1, 2)), np.max(aligned_shapes, axis=(0, 1, 2))])
    ...
    trajectories = pca1.fit_transform(aligned_shapes.reshape(np.product(aligned_shapes.shape[:2]), -1)).reshape(
        *aligned_shapes.shape[:2], -1)
    ...
    mean_trajectory, trajectories_gpa = shape_analysis.generalized_procrustes_analysis(trajectories,
                                                                                       scale=scale)
    n_components2 = 6
    pca2 = PCA(svd_solver='full', n_components=n_components2)
    final_space = pca2.fit_transform(trajectories_gpa.reshape(trajectories_gpa.shape[0], -1))
    ...
    for component in range(final_space.shape[1]):
        ...
        trajectories = np.array([mean_trajectory for i in range(len(component_movement))])
        for i in range(len(component_movement)):
            trajectories[i, component] = component_movement[i]
        print(trajectories.shape)
        trajectories_shapes = get_shape_space_from_trajectory_space(trajectories, pca2, trajectories_gpa.shape[1:])
        ...
        plt.savefig(Path(f'results/{names[3]}/trajectory_space_component_{component + 1}_in_shape_space.png'))
        plt.close()
