...
if __name__ == '__main__':
    ...
    trajectory_orientations = [
        get_trajectory_orientation_vector(shape, es_id)
        for shape in shape_space]
    pc1_2_angles = [
        get_angle(vector[[0, 1]]) for vector in trajectory_orientations
    ]
    pc1_3_angles = [
        get_angle(vector[[0, 2]]) for vector in trajectory_orientations
    ]
    pc1_4_angles = [
        get_angle(vector[[0, 3]]) for vector in trajectory_orientations
    ]
    table = pd.read_excel(Path('../../raw_data/Strain_Алмаз.xlsx'), sheet_name=1).set_index('ID')
    table['CS'] = 0.
    table['TS_1'] = 0.
    table['TS_2'] = 0.
    table['TS_3'] = 0.
