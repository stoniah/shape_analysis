...
def plot_explained_variance(pca, out_dir, cumsum=True, title='Shape Space components explained variance'):
    elbow_plot_outfile = out_dir.joinpath(f"{title}.png")
    variance = pca.explained_variance_ratio_
    if cumsum:
        variance = np.cumsum(variance)
    n_components = len(variance)
    component_ids = np.arange(1, n_components + 1)
    plt.plot(component_ids, variance * 100, marker="o")
...
def plot_areas(
        area_graph_dir, contours, areas, areas_time_interpolated, areas_fourier_interpolated
):
    ...
    for j in range(len(contours)):
        for i in range(3):
            ...
            plt.plot(
                time, area_list, color=colors[i], marker=markers[i], fillstyle="none"
            )
...
def pca_animation1(shapes_shape, pca, pca_space, out_dir, n_frames=30, interval=50):
    """
    ...
    plot_explained_variance(pca, out_dir)
    ...
    for component in range(n_components):
        ...
        component_change = np.concatenate(
            [
                component_decrease,
                component_increase,
                component_increase[::-1][1:],
                component_decrease[::-1][:-1],
            ]
        )
        pca_series = np.repeat(
            pca_mean_shape[np.newaxis, :], len(component_change), axis=0
        )
        pca_series[:, component] = component_change
        shape_variation_series = (
            pca.inverse_transform(pca_series)
            .reshape(-1, *shapes_shape[1:])
            .transpose(0, 2, 1)
        )
        distances = np.linalg.norm(
            shape_variation_series - shape_variation_series[n_frames + 1], axis=1
        )
        shape_bounds = np.array(
            [
                np.min(shape_variation_series, axis=(0, 2)),
                np.max(shape_variation_series, axis=(0, 2)),
            ]
        )
        fig, ax = plt.subplots()
        ...
        scalar_mappable = ScalarMappable(
            mcolors.Normalize(vmin=0, vmax=np.max(distances)), "YlOrRd"
        )

        init_func = functools.partial(
            init, ax=ax, line_endo=line_endo, line_epi=line_epi, bounds=shape_bounds
        )
        ...
        frames = [
            (shape_variation_series[i], shape_variation_series[n_frames + 1])
            for i in range(len(shape_variation_series))
        ]
        ani = FuncAnimation(
            fig,
            func=update_func,
...
def plot_trajectory_animation(trajectory, out_path, name, bounds, interval):
    ...
    ani = FuncAnimation(
        fig, func=update_func, frames=trajectory, interval=interval, blit=True
    )
    ani.save(out_path)
    plt.close(fig)


def plot_trajectory_animation_diff(
        trajectory, mean_trajectory, out_path, name, bounds, interval
):
    def update(
            frame,
            line_epi_,
    ...
    scatter = plt.scatter(
        trajectory[0][0],
        trajectory[0][1],
        c=scalar_mappable.to_rgba(distances[0]),
        cmap="YlOrRd",
        s=100,
        vmin=0,
    ...
    update_func = functools.partial(
        update,
        line_epi_=line_epi,
        line_endo_=line_endo,
        line_endo_ref_=line_endo_ref,
        line_epi_ref_=line_epi_ref,
        connecting_lines_=connecting_lines,
        scatter_=scatter,
